...
class Data_Extractor():
    ...
    def Ratio_predictors(self):
        ...
        Ratios_Frame = Ratios_Frame.merge(roe, on=['symbol', 'calendarYear'], how='left').fillna(0.0) ###### DOES IT MAKE SENSE

        Ratios_Frame = Ratios_Frame.loc[:, ~Ratios_Frame.columns.isin(['date_x', 'period_x'])].copy()
...
# Construction Case
company_tick = ['AAPL', 'APA', 'ACGL', 'ANET', 'AXON']
existing_company = []
dataset = []
dictionary_company_data = {}

'''
E1 = Data_Extractor('ANET', 'annual', offset_limit=300)
Data_all = E1.Main_Pipeline()

'''
for i in company_tick:
    print(i)
    if i in existing_company:
        continue

    E1 = Data_Extractor(i, 'annual', offset_limit=300)
    Data_all = E1.Main_Pipeline()
    dictionary_company_data[i] = Data_all.three_cluster

    if len(dataset) == 0:
        dataset = Data_all.three_cluster
    elif len(dictionary_company_data[i]) == 0:
        pass
    else:
        dataset = pd.concat([dataset, Data_all.three_cluster])

    existing_company.append(i)
